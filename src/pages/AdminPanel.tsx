    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="max-w-7xl mx-auto px-3 py-4 sm:px-4 sm:py-8">
        ...
        <ModerationPanel />
          ...
          <>
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-3 mb-8 sm:p-6">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4 sm:text-2xl sm:mb-6">
                {editingPostId ? 'Редактировать пост' : 'Создать новый пост'}
              </h2>

              <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px] lg:gap-6">
                <div className="rounded-2xl border border-gray-200 bg-slate-100 p-2 dark:border-gray-700 dark:bg-gray-900/70 sm:p-5">
                  <div className="overflow-hidden rounded-3xl bg-[#d7e8f7] p-2 shadow-inner dark:bg-slate-950 sm:p-5">
                    <div className="w-full rounded-[1.35rem] rounded-br-md bg-white p-3 shadow-md dark:bg-gray-800 sm:p-5 lg:ml-auto lg:max-w-3xl">
                      <div className="mb-4 flex items-center justify-between gap-3 border-b border-gray-100 pb-3 dark:border-gray-700">
                        <div className="flex items-center gap-2 text-base font-semibold text-blue-600 dark:text-blue-400">
                          <Send size={20} />
                          <span>{editingPostId ? 'Изменение' : 'Пост'}</span>
                        </div>
                      </div>

                      <div className="space-y-4">
                        <div>
                          <label className="mb-2 flex items-center gap-2 text-base font-semibold text-gray-700 dark:text-gray-300">
                            <FileText size={18} />
                            Заголовок
                          </label>
                          <input
                            ...
                            className="w-full border-0 border-b border-gray-200 bg-transparent px-0 py-3 text-xl font-semibold text-gray-900 outline-none placeholder:text-gray-400 focus:border-blue-500 dark:border-gray-700 dark:text-white dark:focus:border-blue-400 sm:text-2xl"
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            rows={10}
                            className="min-h-[220px] w-full resize-y rounded-2xl border border-gray-200 bg-gray-50 px-4 py-4 text-base leading-7 text-gray-900 outline-none transition focus:border-blue-500 focus:bg-white dark:border-gray-700 dark:bg-gray-900 dark:text-white dark:focus:border-blue-400 sm:min-h-[260px]"
                            placeholder="Текст"
                          />
                        </div>

                        <div className="rounded-2xl border border-dashed border-blue-200 bg-blue-50/70 p-3 dark:border-blue-900/60 dark:bg-blue-950/30 sm:p-4">
                          <div className="mb-3 space-y-3">
                            <p className="text-base font-semibold text-gray-800 dark:text-gray-100">Вложения</p>
                            <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
                              {(['photo', 'video', 'audio', 'file'] as ContentType[]).map(type => (
                                <label
                                  key={type}
                                  className="inline-flex h-12 cursor-pointer items-center justify-center gap-2 rounded-full bg-white px-4 text-base font-semibold text-gray-700 shadow-sm ring-1 ring-gray-200 transition hover:bg-blue-600 hover:text-white dark:bg-gray-800 dark:text-gray-200 dark:ring-gray-700 dark:hover:bg-blue-600 sm:h-11"
                                >
                                  {type === 'photo' && <ImageIcon size={16} />}
                                  {type === 'video' && <Video size={16} />}
                                  {type === 'audio' && <Music size={16} />}
                                  {type === 'file' && <FileIcon size={16} />}
                                  {contentTypeLabels[type].toLowerCase()}
                                  <input
                                    type="file"
                                    multiple
                                    accept={
                          ...
                          {[...existingMediaFiles.map((media, index) => ({ ...media, index, existing: true as const })), ...mediaFiles.map((media, index) => ({ type: media.type, filename: media.file.name, size: media.file.size, index, existing: false as const }))].length > 0 ? (
                            ...
                            <label className="flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-gray-300 bg-white/70 px-4 py-8 text-center transition hover:border-blue-400 hover:bg-white dark:border-gray-700 dark:bg-gray-900/70 dark:hover:border-blue-500">
                              ...
                              <span className="text-base font-semibold text-gray-700 dark:text-gray-200">Файл</span>
                              <input
                                type="file"
                                multiple
                                onChange={(e) => {
                ...
                <aside className="space-y-4">
                  <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <label className="mb-3 flex items-start gap-3 rounded-xl bg-gray-50 p-3 text-sm text-gray-700 dark:bg-gray-900 dark:text-gray-300">
                      ...
                      />
                      <span className="block font-semibold">Описание</span>
                    </label>
                    ...
                    <label className="flex items-start gap-3 rounded-xl bg-gray-50 p-3 text-sm text-gray-700 dark:bg-gray-900 dark:text-gray-300">
                      <input
                        ...
                        className="mt-1 h-4 w-4"
                      />
                      <span className="block font-semibold">Комментарии</span>
                    </label>
                  </div>
                  ...
                  <div className="flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <button
                      ...
                      disabled={loading}
                      className="flex h-12 items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 text-base font-semibold text-white transition-colors hover:bg-blue-700 disabled:bg-gray-400"
                    >
                      <Send size={20} />
